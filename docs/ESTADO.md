...
**Estado general:** auditoría completa cerrada + las 5 features nuevas (stock alimento, umbrales ITH, roles, stock medicamentos, movimientos) implementadas y verificadas por HTTP. 29/29 tests pasan. Único pendiente conocido: `iniciar.bat` corre `seed` en cada arranque (idempotente, inofensivo). Falta el chequeo visual en navegador (la extensión de Chrome no estaba conectada en esta sesión).
